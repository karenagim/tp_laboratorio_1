...
int loadASector(eSector listaSec[], int lenSec, char mensaje[],char mensajeError[],int min,int max)
{

    int sectorIngresado= -1;

     if(listaSec!= NULL && lenSec > 0)
     {
          sector_printList(listaSec, lenSec);

          utn_getNumero(&sectorIngresado,mensaje,mensajeError,min,max,3);


          // sectorIngresado =getValidIntLimite(mensaje, mensajeError, min, max,3);
          /*if(!(sectorIngresado >= min && sectorIngresado <=max))
          {
        	  sectorIngresado= -1;
          }*/
     }
